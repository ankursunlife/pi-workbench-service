...
public class SecurityResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		 ...
		 * .and() .authorizeRequests() .antMatchers("/*") .access("#oauth2.hasScope('" +
		 * accessAsUserScope + "')"); // required scope to access /api URL
		 */
		...
		http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().authorizeRequests()
				.antMatchers(AUTH_WHITELIST).permitAll().and().authorizeRequests().anyRequest().authenticated();
	...
	public void configure(ResourceServerSecurityConfigurer resources) {
		...
		// this is because the aud claim is variable in AAD (e.g. clientId or
		// api://clientId ).
