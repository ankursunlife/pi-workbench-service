...
class OboAuthProvider extends BaseAuthenticationProvider {
	...
	public CompletableFuture<String> getAuthorizationTokenAsync(@Nonnull URL url) {
		...
		// Gets incoming access token and generates cache key. The cache key will be
		// used to store
		...
		try {
			...
			OnBehalfOfParameters parameters = OnBehalfOfParameters
					.builder(Collections.singleton(scope), new UserAssertion(authToken)).build();
	...
	/**
	 * Retrieves the access token token included in the incoming request. This
	 * access token will be exchanged for an access token to access Microsoft Graph,
	 * on behalf of the user that is signed in the web application.
	 */
